#!/usr/bin/env bash
# 8-sample parallel benchmark — ken-nipt alignment resource profile
#   bwa_threads=8, samtools sort=4×6G, picard=12G (via run_nipt_v3.sh env)
set -euo pipefail
...
LABCODE=cordlife
WORK_DIR=benchmark_parallel8_ken8_2603
MAX_CPUS=8
SAMTOOLS_THREADS=4
SAMTOOLS_MEMORY=6G
PICARD_MEMORY=12G
RESULTS=${REPO}/tmp/benchmark_parallel8_ken8_results.tsv
PIDS=()
...
rm -f "${REPO}/tmp/benchmark_parallel8_ken8_"*.done 2>/dev/null || true

BATCH_START=$(date -Iseconds)
BATCH_START_SEC=$(date +%s)

echo "=========================================="
echo " 8-sample parallel (ken-like alignment)"
echo " max_cpus=${MAX_CPUS} samtools=${SAMTOOLS_THREADS}x${SAMTOOLS_MEMORY} picard=${PICARD_MEMORY}"
echo " work_dir=${WORK_DIR}  batch_start=${BATCH_START}"
echo "=========================================="
...
echo ""
echo "=========================================="
echo " Batch wall: ${BATCH_MIN} min (${BATCH_WALL}s)"
