...
def plot_genome(df: pd.DataFrame, calls: pd.DataFrame | None,
    ...
    cnv_board.render_board(
        df, calls, prefix, cyto_dict=cyto_dict, log_prefix="plot_gxcnv1",
...
def plot_chromosome(df_chr: pd.DataFrame, calls: pd.DataFrame | None,
    ...
    cnv_board.render_chromosome(
        ...
        log_prefix="plot_gxcnv1", dpi=DPI_CHR, guides=guides,
    )
...
def main():
    ...
    if z_cutoff is None:
        print("[plot_gxcnv1] No z cutoff found — threshold lines omitted", flush=True)
