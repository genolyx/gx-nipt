...
process GXCNV_PREDICT {
    ...
    // Extract group from composite id: "SAMPLE_orig" → grp="orig", base_sid="SAMPLE"
    def _parts   = sample_id.toString().tokenize('_')
    def grp      = _parts[-1]
    """
    set -euo pipefail

    # ── Resolve gender-specific reference ──────────────────────────────────────
    ...
    if [ "${grp}" = "orig" ]; then
        ...
        REF_NPZ="${params.ref_dir}/labs/${labcode}/GXCNV/\${GENDER}_${grp}/reference.npz"
    fi
    echo "[GXCNV] group=${grp}  gender=\${GENDER}  ref=\${REF_NPZ}"

    if [ ! -f "\${REF_NPZ}" ]; then
        echo "[GXCNV] WARNING: reference not found for group '${grp}': \${REF_NPZ}" >&2
        echo "[GXCNV] Skipping — creating empty stub outputs." >&2
        for suffix in bins segments calls regions qcmetrics; do
            printf "##gxcnv_skip=true\\n#chrom\\tstart\\tend\\tregion_name\\n" > ${sample_id}_\${suffix}.tsv
        done
        printf "##gxcnv_skip=true\\npredicted_sex\\tNA\\n" > ${sample_id}_sex.txt
        touch ${sample_id}_genome.png ${sample_id}_regions.png ${sample_id}_qc.png
    ...
    # Remove any PNGs generated by gxcnv itself — GXCNV_PLOT regenerates them
    rm -f ${sample_id}_genome.png ${sample_id}_regions.png ${sample_id}_qc.png
    """
...
// ─────────────────────────────────────────────────────────────────────────────
// Step 5: Plot gx-cnv results
//         Genome-wide + per-chromosome Z-score figures (replaces built-in PNGs).
// ─────────────────────────────────────────────────────────────────────────────
process GXCNV_PLOT {
    tag "${sample_id}"
    label 'process_low'
    ...
    tuple val(sample_id), path("${sample_id}_regions.png"),  emit: regions_png
    ...
    python3 /opt/gx-nipt/bin/scripts/plot_gxcnv.py \\
        --bins  ${bins_tsv} \\
        --calls ${calls_tsv} \\
        -o      ${sample_id}
    ...
    stub:
    """
    touch ${sample_id}_genome.png ${sample_id}_regions.png \\
          ${sample_id}_qc.png
    """
}
